...
async def create_client(
    ...
    if nodes_list is None:
        nodes_list = [
            ("glide-serverless-valkey-ime4qh.serverless.use1.cache.amazonaws.com", 6379)
        ]
...
async def app_logic(client: GlideClusterClient):
    ...
    Logger.log(Level.INFO, "app", f"Set response is = {set_response!r}")

    get_response = await client.get("foo")
    assert isinstance(get_response, bytes)
    Logger.log(Level.INFO, "app", f"Get response is = {get_response.decode()!r}")

    # Send PING to all primaries (according to Redis's PING request_policy)
    pong = await client.ping()
    Logger.log(Level.INFO, "app", f"PING response is = {pong!r}")
...
async def exec_app_logic():
    ...
    while True:
        try:
            ...
            Logger.log(Level.ERROR, "glide", f"TimeoutError encountered: {e}")
            raise e
        except ConnectionError as e:
            # The client wasn't able to reestablish the connection within the given retries
            Logger.log(Level.ERROR, "glide", f"ConnectionError encountered: {e}")
            raise e
        except RequestError as e:
            # Other error reported during a request, such as a server response error
            Logger.log(Level.ERROR, "glide", f"RequestError encountered: {e}")
            raise e
        except Exception as e:
            Logger.log(Level.ERROR, "glide", f"Unexpected error: {e}")
            raise e
        finally:
            try:
                await client.close()
